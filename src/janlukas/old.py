...
def incorporate_local_map(
    knight_pos: (int, int), knight_range: int, local_map: np.ndarray
):
    ...
    if local_map.shape[1] == 2 * knight_range + 1:
        ...
        if knight_y < knight_range:
            ...
            local_y_start = knight_y - knight_range
    global_slice = GLOBAL_MAP[
        local_x_start : local_x_start + local_map.shape[0],
...
class Slayer(BaseAI):
    def __init__(self, *args, **kwargs):
        ...
        if kwargs:
            ...
            self.seeking = False
            self.seek_targets = (
                TARGETS_RED_SEEK if kwargs["team"] == "red" else TARGETS_BLUE_SEEK
            )
            self.tick = 0
            if self.index < 2:
    ...
    def random_path(self, pos, speed):
        self.path = []
        for _ in range(3):
            pos = pos + direction_from_heading(
                np.random.random() * 360.0, speed
            ).astype(int)
    ...
    def circle(self, pos):
        ...
        for angle, delta in zip(
            (0, 180, 270, 90), ((80, 0), (-80, 0), (0, 80), (0, -80)), strict=True
        ):
...
team = {"Gleidi Geschmeidi": Slayer, "Mme. Vajine": Slayer, "Sir Cumbert": Slayer}
